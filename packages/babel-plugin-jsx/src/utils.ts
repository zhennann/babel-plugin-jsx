...
export const createIdentifier = (state: State, name: string): t.Identifier | t.MemberExpression => state.get(name)();
...
export const shouldTransformedToSlots = (tag: string) => !new RegExp(String.raw`^_?${FRAGMENT}\d*$`).test(tag) && tag !== KEEP_ALIVE;
...
export const getBehaviorTagName = (path: NodePath<t.JSXOpeningElement>) => {
  const typePath = path.get('attributes').find(attribute => {
    ...
    return attribute.get('name').isJSXIdentifier() && attribute.get('name').node.name === 'behaviorTag';
  }) as NodePath<t.JSXAttribute> | undefined;
  const node = typePath ? (typePath.get('value').node as t.JSXExpressionContainer) : null;
  if (!node) return null;
 ...
 */
export const getTag = (path: NodePath<t.JSXElement>, state: State): t.Identifier | t.CallExpression | t.StringLiteral | t.MemberExpression => {
 ...
 */
export const transformJSXText = (path: NodePath<t.JSXText | t.StringLiteral>): t.StringLiteral | null => {
 ...
 */
export const transformJSXSpreadChild = (path: NodePath<t.JSXSpreadChild>): t.SpreadElement => t.spreadElement(path.get('expression').node);
...
const mergeAsArray = (existing: t.ObjectProperty, incoming: t.ObjectProperty) => {
  if (t.isArrayExpression(existing.value)) {
    ...
    existing.value = t.arrayExpression([existing.value as t.Expression, incoming.value as t.Expression]);
  }
};
 ...
 * @returns boolean
 */
export const isConstant = (node: t.Expression | t.Identifier | t.Literal | t.SpreadElement | null): boolean => {
