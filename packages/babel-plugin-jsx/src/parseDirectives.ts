...
const getType = (path: NodePath<t.JSXOpeningElement>) => {
  const typePath = path.get('attributes').find(attribute => {
    ...
    return attribute.get('name').isJSXIdentifier() && (attribute.get('name') as NodePath<t.JSXIdentifier>).node.name === 'type';
...
const parseModifiers = (value: any): string[] =>
  t.isArrayExpression(value) ? value.elements.map(el => (t.isStringLiteral(el) ? el.value : '')).filter(Boolean) : [];
  ...
  return {
    ...
    directive: shouldResolve
      ? ([
          ...
          !!modifiersSet[0]?.size &&
            t.objectExpression([...modifiersSet[0]].map(modifier => t.objectProperty(t.identifier(modifier), t.booleanLiteral(true)))),
        ].filter(Boolean) as t.Expression[])
      : undefined,
  };
};

const resolveDirective = (path: NodePath<t.JSXAttribute>, state: State, tag: Tag, directiveName: string) => {
  ...
  if (directiveName === 'model') {
    ...
    let tagName = t.isStringLiteral(tag) ? (tag as t.StringLiteral).value : (tag as t.Identifier).name;
    if (tagName === 'ZBehavior__') {
      const _tagName = getBehaviorTagName(path.parentPath as NodePath<t.JSXOpeningElement>);
